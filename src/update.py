...
def selectTraces(dados: Tuple[go.Figure, ...], selected_option: str) -> Tuple[Tuple[go.Figure, ...], Tuple[float, float]]:
    ...
    if selected_option == 'circle':
        ...
        dados = [trace for trace in dados if 'segments_a' in trace['name'] or 'segments_b' in trace['name']
                  or 'f1(x)' in trace['name'] or 'f2(x)' in trace['name'] or 'y=x' in trace['name']
                    or 'f1_inv(x)' in trace['name'] or 'f2_inv(x)' in trace['name']]
    elif selected_option == 'segments_a':
        range = df.segment_scale
        dados = [trace for trace in dados if 'segments_a' in trace['name'] or 'f1(x)' in trace['name']
                  or 'f2(x)' in trace['name'] or 'y=x' in trace['name'] or 'f1_inv(x)' in trace['name'] or 'f2_inv(x)' in trace['name']]
    elif selected_option == 'segments_b':
        range = df.segment_scale
        dados = [trace for trace in dados if 'segments_b' in trace['name'] or 'f1(x)' in trace['name']
                  or 'f2(x)' in trace['name'] or 'y=x' in trace['name'] or 'f1_inv(x)' in trace['name'] or 'f2_inv(x)' in trace['name']]
    elif selected_option == 'a':
        range = df.all_scale
        dados = [trace for trace in dados if 'segments_a' in trace['name'] or 'points_a' in trace['name']
                  or 'f1(x)' in trace['name'] or 'f2(x)' in trace['name'] or 'y=x' in trace['name']
                    or 'circle' in trace['name'] or 'f1_inv(x)' in trace['name'] or 'f2_inv(x)' in trace['name']]
    elif selected_option == 'b':
        range = df.all_scale
        dados = [trace for trace in dados if 'segments_b' in trace['name'] or 'points_b' in trace['name']
                  or 'f1(x)' in trace['name'] or 'f2(x)' in trace['name'] or 'y=x' in trace['name']
                    or 'circle' in trace['name'] or 'f1_inv(x)' in trace['name'] or 'f2_inv(x)' in trace['name']]
